...
describe(`Password definition`, () => {
    ...
    const passwordDef = _.n('myPassword').password({
        regexp: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).*$/, 
        minLength:8,
        maxLength:30,
        ...
        it('throws an error if password is less than 8 characters', async () => {
            await expect(passwordDef.formatAndValidate('>8Char')).rejects.toThrowError(
                'Password is inferior than minLength of 8'
            );
        })

        it('throws an error if password is less than 8 characters', async () => {
            await expect(passwordDef.formatAndValidate('>30Char1234567891011121314151617181920')).rejects.toThrowError(
                'Password is superior than maxLength of 30'
            );
        })
})